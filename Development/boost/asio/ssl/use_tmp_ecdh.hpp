    ...
    const boost::asio::const_buffer& certificate, boost::system::error_code& ec)
{
    ...
    bio_cleanup bio = { ::BIO_new_mem_buf(const_cast<unsigned char*>(static_cast<const unsigned char*>(certificate.data())), static_cast<int>(boost::asio::buffer_size(certificate))) };
    if (bio.p)
    {
        return do_use_tmp_ecdh(ctx, bio.p, ec);
    }
